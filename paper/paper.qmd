...
author: 
  ...
  - Yunzhao Li
  - Wentao Sun
thanks: "Code and data are available at: https://github.com/yunzhaol/Impact_regret.git. A replication of various aspects in this paper are available at: https://doi.org/10.1080/02699931.2018.1504747"
date: today
date-format: long
...
we use R [@citeR] for all data wrangling and analysis and R packages ggplot2 [@ggplot2], scales [@scales],knitr [@knitr], MBESS [@MBESS], psych [@psych], dplyr [@dplyr], Hmisc [@Hmisc], effsize [@effsize], jmv [@jmv], Rcpp [@Rcpp], reshape2 [@reshape2], readr [@readr], janitor [@janitor], tidyverse [@tidyverse] to read in, clean and modify the data, as well as view statistical information by conducting binomial test. Furthermore, produce the tables and figures to show our results. 

The remainder of this paper is structured as follows. @sec-data In the Data part, we detail the methodology behind our data collection and analysis, and describe the survey data used. @sec-results In the Result part, We employ plots and textual analysis to present our findings in two scenario, illustrating the relationship between routine, regret, social norms, and negative affect across different contexts. @sec-dicussion Moving beyond the raw data, the Discussion part interprets our findings within a broader psychological and societal framework. @sec-conclusion In the conclusion part, we summarize our key discoveries, how our results contribute to the existing body of knowledge on regret and routine behaviors. @sec-appendixIn the appendix, the processes of simulating, reading, cleaning, gathering statistical insights, and testing the data are comprehensively outlined.The figures and tables are based on the data collected and detailed in the appendix section that aggregates statistical information.
...
Given scenario: Mr. Jones almost never takes hitch-hikers in his car. Yesterday he gave a man a ride and was robbed. Mr. Smith frequently takes hitch-hikers in his car. Yesterday he gave a man a ride and was robbed. (Lucas Kutscher & Gilad Feldman (2019))

## Measure
The experimental design consisted of two parts, both replications of the scenarios and data presented in the Kahneman and Miller (1986) paper - the hitchhiking scenario and the car accident scenario.

In the Part1, Participants answered two questions regarding perceived social norms, measuring descriptive norms –“whose behaviour do you think is more common in society? (Mr. Jones / Mr. Smith)” and injunctive norms – “whose behaviour do you think will be more criticised by others in society? (Mr. Jones / Mr. Smith)”(Lucas Kutscher & Gilad Feldman (2019))
...
Table 1 is a replication of table 1 in the original paper, and it shows a total of 342 counts and proportions for perceived regret, social norms, and negative affect in the hitch-hiking Experiment 1. 
...
Figure2 is a replication of figure2 in the original paper, showing the counts and proportions of perceived regret and luck for car accident events. 

Table2 shows that most participants regretted the exception (277, 81%) more strongly than the rule (65,19%; p < .001, d = 1.58). The same result can be seen more intuitively in figure2. the result for the exception event represented by Mr. White is much higher for both Regret and Luck than for the Routine event represented by Mr. Adams.
...
## Implications of Regret Research
...
We replicated two parts of the (2021) paper-the hitchhiking scenario and the car accident scenario. Overall, through participant data analyses of regret, luck, social norms, and negative affect in different scenarios, we all gained support for estimand: people feel more regret when the event occurs in exception scenarios than when they are harmed in routine events.
...
We acquired a CSV file containing raw data relevant to 'Routines and Regret: An Examination of Behavioral Norms and Emotional Responses.' Subsequently, we inspected the first six rows and assessed the demographic details.
...
dataset_selected <- data %>% select(Sc1_regret, sc1_socnorms1, sc1_socnorms2, sc1_combinednorms) 
...
## Clean and test data for statistical information 
